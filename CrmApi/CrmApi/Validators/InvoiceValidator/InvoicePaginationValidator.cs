...
{
    public class InvoicePaginationValidator : AbstractValidator<InvoicePaginationDto>
    {
        public InvoicePaginationValidator()
        {
            RuleFor(x => x.Page)
            ...
            RuleFor(x => x.Status)
                .Must(s => string.IsNullOrEmpty(s) || new[] { "Gönderildi", "Kısmen Ödendi", "Ödendi", "Gecikmiş", "İptal" }.Contains(s))
