...
const fetchCampaigns = async () => {
  try {
    // 🔥 DOĞRU ENDPOINT: /api/leads/campaigns
    const response = await api.get('/leads/campaigns');
    console.log('📊 Kampanya listesi:', response.data);
    
    // Eğer response.data direkt array ise
    if (Array.isArray(response.data)) {
      setCampaigns(response.data);
    } 
    // Eğer response.data.data içinde ise
    else if (response.data.data && Array.isArray(response.data.data)) {
      setCampaigns(response.data.data);
    } 
    // Eğer response.data içinde başka bir yapı varsa
    else if (response.data.$values && Array.isArray(response.data.$values)) {
      setCampaigns(response.data.$values);
    }
  } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Lead oluşturulamadı');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-3xl">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Yeni Potansiyel Müşteri</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            Yeni lead ekleyin ve takip edin
          </p>
        </div>
